/**
 * Simulação da execução do teste de acesso ao processo - CENÁRIO DE FALHA
 * Este script simula o fluxo de teste que seria executado no n8n quando o processo não é encontrado
 */

const fs = require('fs');
const path = require('path');

// Função para carregar a resposta simulada da API Datajud (caso de falha)
function carregarRespostaSimulada() {
  try {
    const dadosArquivo = fs.readFileSync(
      path.join(__dirname, 'simulacao-resposta-falha.json'),
      'utf8'
    );
...
function consultarProcesso() {
  ...
  console.log('⚠️ SIMULANDO CENÁRIO DE FALHA: Processo não encontrado');
  
  // Em uma execução real, aqui seria feita uma chamada à API do Datajud
  // Nesta simulação, usamos a resposta pré-definida de falha
...
function executarFluxo() {
  console.log('🚀 INICIANDO SIMULAÇÃO DO TESTE DE ACESSO AO PROCESSO (FALHA)');
  console.log('---------------------------------------------------');
